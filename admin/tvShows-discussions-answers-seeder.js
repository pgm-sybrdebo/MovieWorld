...
(async () => {
  ...
  // Get all tv-shows 
  const tvShowsRef = db.collection('tv-shows');
  const tvShows = await (await tvShowsRef.get()).docs.map(
    (doc) => {
      return {
        id: doc.id,
        ...doc.data()
  ...
  tvShows.forEach(tvShow => {
    ...
    for (let i = 0; i < numDiscussions; i++) {
      ...
      tvShowsRef.doc(tvShow.id).collection('discussions').doc(userId).set(
        {
      ...
      if (Math.random() < 0.85) {
        ...
        for (let i = 0; i < numAnswers; i++) {
          ...
          tvShowsRef.doc(tvShow.id).collection('discussions').doc(userId).collection('discussions').doc(userId2).set(
            {
              title: faker.lorem.sentence(),
              discussion: faker.lorem.paragraphs(generateValueBetweenMinAndMax(1, 5)),
              ...generateTimestamps()
          ...
          if (Math.random() < 0.45) {
            tvShowsRef.doc(tvShow.id).collection('discussions').doc(users.users[0].uid).collection('discussions').doc(users.users[1].uid).collection('discussions').doc(users.users[1].uid).set(
              {
                title: faker.lorem.sentence(),
                discussion: faker.lorem.paragraphs(generateValueBetweenMinAndMax(1, 5)),
                ...generateTimestamps()
